...
func (t *HyperliquidTrader) SyncOrdersFromHyperliquid(traderID string, exchangeID string, exchangeType string, st *store.Store) error {
	...
	for _, trade := range trades {
			// 【归属权过滤】查找原始订单进行验证
			// 由于 Hyperliquid API 不返回 OrderID，使用 TradeID 查找原始订单
	...
	go func() {
		for {
			...
			case <-t.orderSyncStopChan:
				...
				logger.Infof("🔄 Hyperliquid order sync stopped for trader %s", traderID)
				return
...
func (t *HyperliquidTrader) StopOrderSync() {
