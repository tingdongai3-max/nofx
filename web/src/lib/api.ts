...
export const api = {
  ...
  async getExperiments(window = '72h'): Promise<ColliderExperimentsResponse> {
    const result = await httpClient.get<ColliderExperimentsResponse>(
      `${API_BASE}/experiments?window=${encodeURIComponent(window)}`
    )
    if (!result.success) throw new Error('获取实验列表失败')
  ...
  async createExperiment(
    ...
    const result = await httpClient.post<CreateExperimentResponse>(
      `${API_BASE}/experiments/create`,
      {
        trader_id: masterTraderId,
        variant_count: variantCount,
      }
    )
    if (!result.success) throw new Error(result.message || '创建实验失败')
    return result.data!
  },

  async getCoachConfig(): Promise<CoachConfig> {
    const result = await httpClient.get<CoachConfig>(
      `${API_BASE}/experiments/coach/config`
    )
    if (!result.success) throw new Error('获取教练配置失败')
    return result.data ?? { coach_model_id: '' }
  },

  async updateCoachConfig(modelId: string): Promise<CoachConfig> {
    const result = await httpClient.post<CoachConfig>(
      `${API_BASE}/experiments/coach/config`,
      { coach_model_id: modelId }
    )
    if (!result.success) throw new Error(result.message || '保存教练配置失败')
    return result.data ?? { coach_model_id: modelId }
  },

  async triggerCoachEvolution(): Promise<void> {
    const result = await httpClient.post(
      `${API_BASE}/experiments/coach/trigger`
    )
    if (!result.success) throw new Error(result.message || '触发手动进化失败')
  },
  ...
  async reloadTrader(traderId: string): Promise<void> {
    const result = await httpClient.post(
      `${API_BASE}/traders/${traderId}/reload`
    )
  ...
  async resetTraderData(traderId: string): Promise<void> {
    const result = await httpClient.post(
      `${API_BASE}/traders/${traderId}/reset_data`
    )
  ...
  async screenerFilter(
    filters: ScreenerFilterCondition[]
  ): Promise<ScreenerFilterResponse> {
