...
function RealtimeValue({
  ...
  const textShadow =
    flashDirection === 'up'
      ? '0 0 12px rgba(14,203,129,0.45)'
      : flashDirection === 'down'
        ? '0 0 12px rgba(246,70,93,0.45)'
        : colorMode === 'signed'
          ? value >= 0
            ? '0 0 10px rgba(14,203,129,0.25)'
            : '0 0 10px rgba(246,70,93,0.25)'
          : 'none'

  return (
    <span
      className={`font-bold transition-all duration-200 ${flashTone}`}
      style={{
        textShadow,
        transform: flashDirection ? 'scale(1.03)' : 'scale(1)',
...
function formatTrailingPct(value: number | undefined): string {
...
export function TraderDashboardPage({
  ...
  const [indicatorAnalysis, setIndicatorAnalysis] =
    useState<IndicatorAnalysisResult | null>(null)
  const [indicatorLoading, setIndicatorLoading] = useState<boolean>(false)
  const [indicatorError, setIndicatorError] = useState<string | null>(null)
  type IndicatorSideFilter = 'all' | 'long' | 'short'
  const [indicatorSideFilter, setIndicatorSideFilter] =
    useState<IndicatorSideFilter>('all')
  const trackedSymbols = useMemo(
    () => positions?.map((position) => position.symbol).filter(Boolean) || [],
    [positions]
  )
  const { realtimePrices } = useBinanceTicker(trackedSymbols)

  const staticWalletBalance = useMemo(() => {
    const walletBalance = Number(account?.wallet_balance)
    if (Number.isFinite(walletBalance)) {
      return walletBalance
    }

    const totalEquity = Number(account?.total_equity) || 0
    const unrealizedProfit = Number(account?.unrealized_profit) || 0
    return totalEquity - unrealizedProfit
  }, [
  ...
  const totalDynamicUpnl = useMemo(() => {
    ...
    return positions.reduce((sum, pos) => {
      ...
      const realtimePrice = realtimePrices[symbolKey]
      const fallbackMarkPrice = Number(pos.mark_price) || 0
      const markPrice = Number.isFinite(realtimePrice)
        ? realtimePrice
        : fallbackMarkPrice
      const side = (pos.side ?? 'long').toString().toLowerCase()
      const direction = side === 'short' ? -1 : 1

      if (entryPrice > 0 && quantity > 0 && markPrice > 0) {
        return sum + (markPrice - entryPrice) * quantity * direction
      }
  ...
  const dynamicTotalEquity = staticWalletBalance + totalDynamicUpnl
  const dynamicTotalEquityPct = useMemo(() => {
    const initialBalance = Number(account?.initial_balance) || 0
    if (initialBalance <= 0) {
      return 0
    }
    return ((dynamicTotalEquity - initialBalance) / initialBalance) * 100
  }, [account?.initial_balance, dynamicTotalEquity])

  const dynamicUpnlPct = useMemo(() => {
    if (staticWalletBalance <= 0) {
      return 0
    }
    return (totalDynamicUpnl / staticWalletBalance) * 100
  }, [staticWalletBalance, totalDynamicUpnl])
    ...
    const fetchAnalysis = async () => {
      ...
      try {
        ...
        const resp = await fetch(
          `/api/statistics/indicator-analysis?${params.toString()}`,
          {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
          }
        )
        const data = (await resp.json()) as IndicatorAnalysisResult & {
          error?: string
          code?: string
          details?: string
        }
  ...
  const handleResetTraderData = async () => {
    ...
    try {
      ...
      notify.success(
        language === 'zh' ? '已软重置，旧数据已隔离隐藏' : 'Soft reset applied'
      )
    } catch (err: unknown) {
      const errorMsg =
        err instanceof Error
      ...
      <div className="w-full px-4 md:px-8 relative z-10 pt-6">
          ...
          <div className="flex items-center justify-end">
            <button
              ...
              title={
                language === 'zh'
                  ? '清空历史数据并重置交易员内存状态'
                  : 'Clear trader history and reset runtime state'
              }
            >
              {resettingTraderData ? (
        ...
        {account && (
          <div className="mb-4 px-3 py-1.5 rounded bg-black/40 border border-white/5 text-[10px] font-mono text-nofx-text-muted flex justify-between items-center opacity-60 hover:opacity-100 transition-opacity">
            ...
            <div className="flex gap-4">
              ...
              <span>EQ::{dynamicTotalEquity.toFixed(2)}</span>
              <span>UPNL::{totalDynamicUpnl.toFixed(2)}</span>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <StatCard
            ...
            value={
              <RealtimeValue
                value={dynamicTotalEquity}
                formatter={(value) => value.toFixed(2)}
                colorMode="neutral"
              />
            }
            unit="USDT"
            change={dynamicTotalEquityPct}
            positive={dynamicTotalEquityPct >= 0}
            icon="💰"
          />
          <StatCard
          ...
          <StatCard
            ...
            unit="USDT"
            change={dynamicUpnlPct}
            positive={totalDynamicUpnl >= 0}
            icon="📈"
          />
          <StatCard
                    ...
                    <table className="w-full text-xs">
                      <thead className="text-left border-b border-white/5">
                        <tr>
                          ...
                          <th className="px-1 pb-3 font-semibold text-nofx-text-muted whitespace-nowrap text-right hidden md:table-cell">
                            {language === 'zh' ? '收益率' : 'ROE %'}
                          </th>
                          <th
                            className="px-1 pb-3 font-semibold text-nofx-text-muted whitespace-nowrap text-right hidden md:table-cell"
                      ...
                      <tbody>
                        {paginatedPositions.map((pos, i) => {
                          ...
                          const markPrice = Number.isFinite(realtimePrice)
                            ? realtimePrice
                            : Number(pos.mark_price) || 0
                          const quantity = Number(pos.quantity) || 0
                          const leverage = Number(pos.leverage) || 0
                          const liquidationPrice =
                            Number(pos.liquidation_price) || 0
                          ...
                          const hasSafetyFloor =
                            ...
                            ((side === 'long' && safetyFloorSL > stopLoss) ||
                              (side === 'short' &&
                                (stopLoss <= 0 || safetyFloorSL < stopLoss)))
                          const hasTrailing =
                            trailingActivationPct > 0 && trailingRetracePct > 0
                          const unrealizedPnl =
                            entryPrice > 0
                              ? (markPrice - entryPrice) * quantity * direction
                              ...
                              <td className="px-1 py-3 font-mono font-semibold whitespace-nowrap text-left text-nofx-text-main group-hover/row:text-white transition-colors">
                                <div className="flex flex-col gap-1">
                                  ...
                                  {(stopLoss > 0 || takeProfit > 0) && (
                                    <span className="text-[10px] text-sky-300/80 font-medium whitespace-nowrap">
                                      {language === 'zh'
                                  ...
                                  {hasSafetyFloor && (
                                    <span className="text-[10px] text-emerald-300/80 font-medium whitespace-nowrap">
                                      {language === 'zh'
                                        ? `🛡️ 护盘 ${formatPrice(safetyFloorSL)}`
                                        : `🛡️ Guard ${formatPrice(safetyFloorSL)}`}
                                    </span>
                                  )}
                                </div>
                              </td>
        ...
        {selectedTraderId && (
          <div
            className="flex flex-col gap-6 animate-slide-in w-full"
          ...
          >
              ...
              <div className="border-t border-white/5 pt-3 mt-2">
                ...
                {indicatorError && !indicatorLoading && (
                  <div className="text-xs text-nofx-red">
                    {language === 'zh'
                      ? '指标分析加载失败：'
                      : 'Failed to load indicator analysis: '}
                    {indicatorError}
                  </div>
                )}
                {indicatorAnalysis &&
                  indicatorAnalysis.trade_count > 0 &&
                  !indicatorLoading &&
                  !indicatorError && (
                    <>
                      ...
                      {(() => {
                        ...
                        const config: {
                          ...
                          {
                            key: 'bias',
                            label: language === 'zh' ? '乖离率 (Bias)' : 'Bias',
                            unit: '%',
                            signed: true,
                          },
                          {
                            ...
                            label:
                              language === 'zh'
                                ? `放量 (前${volMultBars}根)`
                                : `Vol Mult (${volMultBars})`,
                            unit: 'x',
                            volMult: true,
                          },
                          {
                            key: 'obv',
                            label: language === 'zh' ? 'OBV (量价累积)' : 'OBV',
                            unit: '',
                            signed: true,
                          },
                          {
                            key: 'poc_deviation_pct',
                            label:
                              language === 'zh'
                                ? 'POC 偏离度'
                                : 'POC Deviation',
                        ...
                        const hasAny = (d: IndicatorDimensionAverages) =>
                          (d?.profit_entry_count ?? 0) +
                            (d?.profit_exit_count ?? 0) +
                            (d?.loss_entry_count ?? 0) +
                            (d?.loss_exit_count ?? 0) >
                          0
                        ...
                        const BarRow = ({
                          ...
                          const safeBarPct = Number.isFinite(barPct)
                            ? Math.min(100, Math.max(0, barPct))
                            : 0
                          const safeMedPct = Number.isFinite(medianBarPct)
                            ? Math.min(100, Math.max(0, medianBarPct))
                            : NaN
                          const showMedLine =
                            hasMed &&
                            !Number.isNaN(safeMedPct) &&
                            safeMedPct >= 0 &&
                            safeMedPct <= 100
                          return (
                            <div className="min-w-0">
                              <div
                                className="font-mono text-lg font-bold flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-1.5"
                                style={{ color: '#EAECEF' }}
                              >
                                {hasVal ? (
                                  <>
                              ...
                              <div className="h-2 w-full rounded-full overflow-hidden bg-white/10 relative">
                                {hasVal && safeBarPct > 0 ? (
                                  ...
                                  <div
                                    className="h-full w-full rounded-full border border-dashed border-[#2B3139]/80 bg-transparent absolute inset-0"
                        ...
                        return (
                          <div
                            key={indicatorSideFilter}
                            className="grid grid-cols-1 gap-4 animate-fade-in w-full"
                          >
                            {config
                              .filter((item) => {
                              ...
                              .map((row) => {
                                ...
                                const entryWin =
                                  data.profit_entry_count > 0
                                    ? data.profit_entry_avg
                                    : undefined
                                const entryWinMed =
                                  data.profit_entry_count > 0
                                    ? data.profit_entry_median
                                    : undefined
                                const entryLoss =
                                  data.loss_entry_count > 0
                                    ? data.loss_entry_avg
                                    : undefined
                                const entryLossMed =
                                  data.loss_entry_count > 0
                                    ? data.loss_entry_median
                                    : undefined
                                const exitWin =
                                  data.profit_exit_count > 0
                                    ? data.profit_exit_avg
                                    : undefined
                                const exitWinMed =
                                  data.profit_exit_count > 0
                                    ? data.profit_exit_median
                                    : undefined
                                const exitLoss =
                                  data.loss_exit_count > 0
                                    ? data.loss_exit_avg
                                    : undefined
                                const exitLossMed =
                                  data.loss_exit_count > 0
                                    ? data.loss_exit_median
                                    : undefined
                                const allVals = [
                                  entryWin,
                                  entryLoss,
                                ...
                                if (row.signed) {
                                  const min = allVals.length
                                    ? Math.min(...allVals, -1.5)
                                    : -1.5
                                  const max = allVals.length
                                    ? Math.max(...allVals, 1.5)
                                    : 1.5
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="min-w-0 flex flex-col gap-5">
                                      ...
                                      <BarRow
                                        avgValue={entryWin}
                                        medianValue={entryWinMed}
                                        unit={row.unit}
                                        isWin={true}
                                        barPct={entryWinPct}
                                        medianBarPct={entryWinMedPct}
                                        biasSuffix={
                                          row.key === 'bias'
                                            ? formatBiasSuffix(entryWin)
                                            : undefined
                                        }
                                      />
                                      <BarRow
                                        avgValue={entryLoss}
                                        medianValue={entryLossMed}
                                        unit={row.unit}
                                        isWin={false}
                                        barPct={entryLossPct}
                                        medianBarPct={entryLossMedPct}
                                        biasSuffix={
                                          row.key === 'bias'
                                            ? formatBiasSuffix(entryLoss)
                                            : undefined
                                        }
                                      />
                                    </div>
                                    ...
                                    <div className="min-w-0 flex flex-col gap-5">
                                      ...
                                      <BarRow
                                        avgValue={exitWin}
                                        medianValue={exitWinMed}
                                        unit={row.unit}
                                        isWin={true}
                                        barPct={exitWinPct}
                                        medianBarPct={exitWinMedPct}
                                        biasSuffix={
                                          row.key === 'bias'
                                            ? formatBiasSuffix(exitWin)
                                            : undefined
                                        }
                                      />
                                      <BarRow
                                        avgValue={exitLoss}
                                        medianValue={exitLossMed}
                                        unit={row.unit}
                                        isWin={false}
                                        barPct={exitLossPct}
                                        medianBarPct={exitLossMedPct}
                                        biasSuffix={
                                          row.key === 'bias'
                                            ? formatBiasSuffix(exitLoss)
                                            : undefined
                                        }
                                      />
                                    </div>
            ...
            <div className="nofx-glass p-6 w-full">
              ...
              <PositionHistory
                key={`${selectedTraderId}-${dataRefreshNonce}`}
                traderId={selectedTraderId}
              />
            </div>
          </div>
