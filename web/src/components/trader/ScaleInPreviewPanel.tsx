...
import { Activity, AlertTriangle, ChevronDown, ChevronUp, TrendingUp } from 'lucide-react'
import { api } from '../../lib/api'
import type { ScaleInReconcilePreview } from '../../types'

interface ScaleInPreviewPanelProps {
  traderId: string
...
export function ScaleInPreviewPanel({
  traderId,
  refreshInterval = 8000,
  selectedSymbol,
}: ScaleInPreviewPanelProps) {
  const [preview, setPreview] = useState<ScaleInReconcilePreview | null>(null)
  ...
  const loadPreview = useCallback(async () => {
    try {
      const data = await api.getRuntimeScaleInPreview(traderId, selectedSymbol, true)
      setPreview(data)
      setError(null)
    } catch (err) {
  ...
  if (!preview) {
    return (
      <div className="p-3 text-center text-xs rounded" style={cardStyle}>
        No scale-in data available.
      </div>
    )
  }

  const plan = preview.scale_in_plan
  const levels = preview.scale_in_levels ?? []
  const events = preview.scale_in_event_logs ?? []
  ...
  const blockReasons = preview.scale_in_block_reasons ?? []
    ...
    <div className="rounded-lg overflow-hidden" style={cardStyle}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider" style={{ background: preview.scale_in_consistency === 'mismatch' ? '#F6465D22' : '#848E9C22', color: preview.scale_in_consistency === 'mismatch' ? '#F6465D' : '#EAECEF' }}>
            {translateTag(preview.scale_in_consistency)}
          </span>
        </div>
        {expanded ? (
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-3">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
            {[
              ['Has Plan', preview.has_scale_in_plan],
              ['Working Orders', preview.has_working_orders],
              ['State Match', !preview.has_state_mismatch],
              ['Protection Rebalanced', preview.position_aggregate?.has_protection ?? preview.protection_summary?.has_protection ?? false],
            ].map(([label, enabled]) => (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
            ...
            <div className="p-2 rounded" style={{ background: '#1E2329' }}>
              ...
              <div className="text-sm font-mono mt-1" style={{ color: '#EAECEF' }}>{formatQty(preview.remaining_scale_in_qty)}</div>
            </div>
            <div className="p-2 rounded" style={{ background: '#1E2329' }}>
              <div className="text-[10px] uppercase tracking-wider" style={{ color: '#848E9C' }}>Executed Qty</div>
              <div className="text-sm font-mono mt-1" style={{ color: '#EAECEF' }}>{formatQty(preview.executed_scale_in_qty)}</div>
            </div>
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
            <div className="p-2 rounded" style={{ background: '#1E2329' }}>
              <div className="text-[10px] uppercase tracking-wider" style={{ color: '#848E9C' }}>Avg Entry Price</div>
              <div className="text-sm font-mono mt-1" style={{ color: '#EAECEF' }}>
                {formatPrice(aggregate?.avg_entry_price ?? 0)}
              </div>
            </div>
            <div className="p-2 rounded" style={{ background: '#1E2329' }}>
              <div className="text-[10px] uppercase tracking-wider" style={{ color: '#848E9C' }}>Scale-In Count</div>
              <div className="text-sm font-mono mt-1" style={{ color: '#EAECEF' }}>
                {preview.scale_in_count ?? 0}
              </div>
            </div>
            <div className="p-2 rounded" style={{ background: '#1E2329' }}>
              <div className="text-[10px] uppercase tracking-wider" style={{ color: '#848E9C' }}>Risk Budget</div>
              <div className="text-sm font-mono mt-1" style={{ color: '#EAECEF' }}>
                {formatQty(preview.risk_budget_remaining)}
              </div>
            </div>
            <div className="p-2 rounded" style={{ background: '#1E2329' }}>
              <div className="text-[10px] uppercase tracking-wider" style={{ color: '#848E9C' }}>Protected Qty</div>
              <div className="text-sm font-mono mt-1" style={{ color: '#EAECEF' }}>
                {formatQty(protectionSummary?.protected_quantity ?? aggregate?.protected_quantity ?? 0)}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
              ...
              {plan ? (
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <div className="font-mono" style={{ color: '#EAECEF' }}>{plan.scale_in_plan_id}</div>
                  </div>
                  <div>
            ...
            <div className="p-3 rounded" style={{ background: '#1E2329' }}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs uppercase tracking-wider" style={{ color: '#848E9C' }}>Risk / Block Reasons</span>
              </div>
              <div className="space-y-1 max-h-64 overflow-auto pr-1">
                {risk?.blocked_reasons?.length ? risk.blocked_reasons.map((reason, index) => (
                  ...
                  <div className="text-xs" style={{ color: '#848E9C' }}>No scale-in block reasons.</div>
                )}
              </div>
            </div>
          </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {levels.length > 0 ? levels.map((level) => (
                      <tr key={`${level.scale_in_plan_id}-${level.level_index}`} className="border-b border-white/5">
                        <td className="px-1 py-2 font-mono text-nofx-text-main">{level.level_index}</td>
                        <td className="px-1 py-2 font-mono text-nofx-text-main">{level.target_type}</td>
                        ...
                        <td className="px-1 py-2 text-center">
                          <span className="px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider" style={{ background: level.status === 'invalid' ? '#F6465D22' : level.status === 'filled' ? '#0ECB8122' : '#848E9C22', color: level.status === 'invalid' ? '#F6465D' : level.status === 'filled' ? '#0ECB81' : '#EAECEF' }}>
                            {level.status}
                          </span>
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td colSpan={7} className="px-1 py-3 text-xs" style={{ color: '#848E9C' }}>
                          No scale-in levels.
                        </td>
                      </tr>
                    )}
            ...
            <div className="p-3 rounded" style={{ background: '#1E2329' }}>
              ...
              <div className="space-y-1 max-h-80 overflow-auto pr-1">
                {events.length > 0 ? events.map((event) => (
                  <div key={`${event.scale_in_plan_id}-${event.created_at}-${event.event_type}`} className="rounded px-2 py-1 text-[11px]" style={{ background: '#0B0E11', color: '#EAECEF', border: '1px solid #2B3139' }}>
                    ...
                    <div className="font-mono text-[10px] mt-1" style={{ color: '#848E9C' }}>
                      level={event.level_index} source={event.event_source}
                    </div>
                  </div>
                )) : (
                  <div className="text-xs" style={{ color: '#848E9C' }}>No scale-in events.</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
            <div className="p-3 rounded" style={{ background: '#1E2329' }}>
              ...
              <div className="space-y-1 max-h-64 overflow-auto pr-1">
                {mismatchReasons.length > 0 ? mismatchReasons.map((reason, index) => (
                  ...
                  <div className="text-xs" style={{ color: '#848E9C' }}>No scale-in mismatches.</div>
                )}
              </div>
            </div>
            ...
            <div className="p-3 rounded" style={{ background: '#1E2329' }}>
              <div className="flex items-center gap-2 mb-2">
                <Activity className="w-3 h-3" style={{ color: '#F0B90B' }} />
                <span className="text-xs uppercase tracking-wider" style={{ color: '#848E9C' }}>Linked Summaries</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <div style={{ color: '#5E6673' }}>Order Working Qty</div>
                  <div className="font-mono" style={{ color: '#EAECEF' }}>{formatQty(orderSummary?.pending_add_qty ?? preview.pending_add_qty)}</div>
                </div>
                <div>
                  <div style={{ color: '#5E6673' }}>Protection Qty</div>
                  <div className="font-mono" style={{ color: '#EAECEF' }}>{formatQty(protectionSummary?.protected_quantity ?? 0)}</div>
                </div>
                <div>
                  <div style={{ color: '#5E6673' }}>Linked Position</div>
                  <div className="font-mono" style={{ color: '#EAECEF' }}>{plan?.linked_position_key ?? aggregate?.symbol ?? '-'}</div>
                </div>
                <div>
                  <div style={{ color: '#5E6673' }}>Scale-In Block Count</div>
                  <div className="font-mono" style={{ color: '#EAECEF' }}>{blockReasons.length}</div>
                </div>
              </div>
            </div>
          </div>
